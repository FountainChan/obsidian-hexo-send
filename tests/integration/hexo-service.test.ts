...
describe("HexoService",()=>{
  ...
  it("prefers the repository-local Hexo CLI through Node",async()=>{
    ...
    const runner=new RecordingRunner();const service=new HexoService(runner,"custom-node");await service.clean(root);await service.generate(root);
  ...
  it("refuses to run when the repository-local Hexo CLI is missing",async()=>{
    root=await fs.mkdtemp(path.join(os.tmpdir(),"hexo-send-cli-missing-"));
    await expect(new HexoService(new RecordingRunner()).clean(root)).rejects.toMatchObject({code:"HEXO_VALIDATION_FAILED"});
  });
});
