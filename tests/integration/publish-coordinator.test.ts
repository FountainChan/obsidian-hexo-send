...
describe("PublishCoordinator", () => {
  ...
  it("runs two Hexo generations, creates one exact commit, and never pushes", async () => {
    ...
    await fs.writeFile(path.join(repository,"README.md"),"base\n"); await gitRun(["add","--","README.md"]); await gitRun(["commit","-m","base"]); await fs.mkdir(path.join(repository,"source","_posts"),{recursive:true});
    ...
    const result = await new PublishCoordinator(runner).execute(plan,environment,{...DEFAULT_SETTINGS,npxExecutable:"fake-npx"},async()=>null,()=>undefined);
...
class RoutingRunner implements ProcessRunner {
  ...
  async run(request: ProcessRequest): Promise<ProcessResult> {
    if (request.executable === "fake-npx") {
