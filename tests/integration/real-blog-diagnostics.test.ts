...
describe("real blog read-only diagnostics",()=>{
  (realBlog ? it : it.skip)("reads the configured Hexo repository without writes",async()=>{
    ...
    expect(environment.abbrlinkInstalled).toBe(true); expect(environment.categories.flat()).toContain("SEO教程"); expect(environment.items.map((item)=>item.key)).toEqual(expect.arrayContaining(["hexo-config","git","node","npx","pre-commit"]));
  },30_000);
});
