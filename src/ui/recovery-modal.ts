...
export class RecoveryModal extends Modal {
  ...
  onOpen(): void {
    ...
    for (const directory of this.directories) {
      ...
      actions.createEl("button", { text:"恢复本次改动", cls:"mod-warning" }).addEventListener("click", ()=>{ void (async()=>{ try { await this.restore(directory); card.remove(); new Notice("已恢复任务写入与暂存区"); } catch(error){ new Notice(error instanceof Error ? error.message : String(error),10000); } })(); });
      actions.createEl("button", { text:"打开日志目录" }).addEventListener("click",()=>this.openDirectory(directory));
      actions.createEl("button", { text:"忽略" }).addEventListener("click",()=>{ void (async()=>{ await TempJobJournal.ignoreAt(directory); card.remove(); })(); });
    }
  }
