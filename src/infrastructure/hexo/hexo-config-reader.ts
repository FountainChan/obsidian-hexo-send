...
export async function readHexoConfig(repositoryPath: string, overrides: { postsDir?: string; seoPostsDir?: string; imagesDir?: string } = {}): Promise<HexoConfigSnapshot> {
  ...
  const config = parse(await safeFs.read("_config.yml")) as Record<string, unknown>;
  const packageJson = JSON.parse(await safeFs.read("package.json")) as { dependencies?: Record<string, string>; devDependencies?: Record<string, string> };
  const sourceDir = assertSafeRelativePath(text(config.source_dir) || "source");
  const postsDir = assertSafeRelativePath(overrides.postsDir || path.posix.join(sourceDir, "_posts"));
  const seoPostsDir = assertSafeRelativePath(overrides.seoPostsDir || path.posix.join(postsDir, "seo"));
  const imagesDir = assertSafeRelativePath(overrides.imagesDir || path.posix.join(sourceDir, "images"));
  const { categories, tags } = await scanTaxonomy(await safeFs.absolute(postsDir));
