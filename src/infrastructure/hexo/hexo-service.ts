...
export class HexoService {
  constructor(private readonly runner: ProcessRunner, private readonly npxExecutable = "npx",private readonly nodeExecutable="node") {}
  ...
  private async runHexo(repositoryPath:string,args:readonly string[],timeoutMs:number,signal?:AbortSignal):Promise<void>{
    ...
    if(local)await this.runner.run({executable:this.nodeExecutable,args:[localCli,...args,"--cwd",repositoryPath],cwd:repositoryPath,timeoutMs,signal});
    else await this.runner.run({executable:this.npxExecutable,args:["hexo",...args,"--cwd",repositoryPath],cwd:repositoryPath,timeoutMs,signal});
  }
  async readAbbrlink(repositoryPath: string, relativePath: string): Promise<string> {
    const content = await fs.readFile(path.join(repositoryPath, ...relativePath.split("/")), "utf8");
